...
type Lang='it'|'en';
const ui={
 it:{home:'Home',journal:'Journal',more:'Potrebbe interessarti anche',sourceNote:'Orari, accessi, prenotazioni e trasporti possono cambiare. Controlla le informazioni variabili sulla fonte ufficiale prima della visita.',official:'Informazioni ufficiali',tip:'Il consiglio di Marghera Venice Apartments',back:'Torna al Journal',tag:'it-IT',breadcrumb:'Percorso di navigazione'},
 en:{home:'Home',journal:'Journal',more:'You may also like',sourceNote:'Times, access rules, reservations and transport arrangements can change. Check variable details with the official source before your visit.',official:'Official information',tip:'Marghera Venice Apartments tip',back:'Back to Journal',tag:'en-GB',breadcrumb:'Breadcrumb'}
} as const;
...
export default function AuditedPrimaryJournalArticle({lang,data}:{lang:Lang;data:JournalArticleData}){
 ...
 return <><Header lang={lang}/><main><article className="editorial-page">{schemas.map((s,i)=><script key={i} type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(s)}}/>)}
  <EditorialHero image={data.image} imageAlt={data.imageAlt} crumbs={[{label:t.home,href:p(lang,'/')},{label:t.journal,href:p(lang,'/journal')},{label:data.title}]} breadcrumbLabel={t.breadcrumb} eyebrow={<>{data.category}{data.eventDate&&<> • {data.eventDate}</>}</>} title={data.title} subtitle={data.description} strongShade={Boolean(data.posterHero)}/>
