...
export default function PublicLocalGuide({lang='it',kind}:{lang?:Lang;kind:'food'|'services'}){
 ...
 <section className="bg-navy px-5 pb-16 pt-36 text-white lg:px-8"><div className="mx-auto max-w-7xl"><nav className="mb-6 flex gap-2 text-sm text-white/70"><Link href={home}>{t.home}</Link><span>›</span><span className="text-gold">{title}</span></nav><p className="text-xs font-black uppercase tracking-[.22em] text-gold">Marghera Venice Apartments</p><h1 className="mt-4 font-serif text-6xl leading-none md:text-8xl">{title}</h1><p className="mt-6 max-w-3xl text-xl text-white/75">{lead}</p></div></section>
 {kind==='services'?<section className="bg-cream py-16"><div className="mx-auto max-w-6xl px-5 lg:px-8"><div className="space-y-14">{sections.map(section=><section key={section.title}><div className="border-b border-navy/15 pb-4"><h2 className="font-serif text-4xl text-navy">{section.title}</h2></div><div>{section.places.map(p=><article key={`${section.title}-${p.name}`} className="grid gap-3 border-b border-navy/10 py-6 last:border-0 md:grid-cols-[minmax(190px,.75fr)_minmax(0,1.5fr)_auto] md:items-center"><div><div className="text-xs font-black uppercase tracking-[.14em] text-gold">{p.type}{p.zone&&<span className="text-navy/40"> · {p.zone}</span>}</div><h3 className="mt-2 font-serif text-2xl text-navy">{p.name}</h3></div><p className="leading-7 text-slate-600">{p.description}</p><div className="flex flex-wrap gap-2 md:justify-end">{p.maps&&<a href={p.maps} target="_blank" rel="noopener noreferrer" className="inline-flex rounded-full border border-navy/20 px-4 py-2 text-sm font-bold text-navy transition hover:bg-navy hover:text-white">{t.maps} ↗</a>}{p.website&&<a href={p.website} target="_blank" rel="noopener noreferrer" className="inline-flex rounded-full bg-gold px-4 py-2 text-sm font-bold text-navy">{t.website} ↗</a>}</div></article>)}</div></section>)}</div><p className="mt-12 border-t border-navy/10 pt-5 text-sm text-slate-600">{t.note}</p></div></section>:
 <section className="bg-cream py-16"><div className="mx-auto max-w-7xl space-y-10 px-5 lg:px-8">{sections.map(section=><section key={section.title}><h2 className="font-serif text-4xl text-navy md:text-5xl">{section.title}</h2><div className="mt-6 grid gap-5 md:grid-cols-2 xl:grid-cols-3">{section.places.map(p=><article key={`${section.title}-${p.name}`} className="rounded-[2rem] bg-white p-6 shadow-soft"><div className="flex flex-wrap gap-2 text-xs font-black uppercase tracking-[.14em]"><span className="text-gold">{p.type}</span>{p.zone&&<span className="text-navy/45">• {p.zone}</span>}</div><h3 className="mt-3 font-serif text-3xl text-navy">{p.name}</h3><p className="mt-3 leading-7 text-slate-600">{p.description}</p><div className="mt-5 flex flex-wrap gap-2">{p.maps&&<a href={p.maps} target="_blank" rel="noopener noreferrer" className="inline-flex rounded-full bg-navy px-5 py-3 text-sm font-bold text-white">{t.maps} ↗</a>}{p.website&&<a href={p.website} target="_blank" rel="noopener noreferrer" className="inline-flex rounded-full bg-gold px-5 py-3 text-sm font-bold text-navy">{t.website} ↗</a>}</div></article>)}</div></section>)}<p className="rounded-2xl border border-navy/10 bg-white p-5 text-sm text-slate-600">{t.note}</p><section className="rounded-[2rem] bg-navy p-7 text-white md:p-9"><p className="text-xs font-black uppercase tracking-[.2em] text-gold">{foodRelated[lang].eyebrow}</p><h2 className="mt-3 font-serif text-4xl">{foodRelated[lang].title}</h2><div className="mt-6 flex flex-wrap gap-3">{foodRelated[lang].items.map(([label,href])=><Link key={href} href={href} className="rounded-full bg-gold px-5 py-3 font-bold text-navy">{label} →</Link>)}</div></section></div></section>}
