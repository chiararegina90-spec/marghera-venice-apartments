...
export default function EventJournalArticle({lang,data}:{lang:EventLang;data:EventArticle}){
  ...
  const eventJsonLd={
    ...
    startDate,endDate,eventStatus:'https://schema.org/EventScheduled',eventAttendanceMode:'https://schema.org/OfflineEventAttendanceMode',location:data.eventLocations?.length?data.eventLocations.map(place=>({'@type':'Place',name:place.name,address:{'@type':'PostalAddress',...(place.streetAddress?{streetAddress:place.streetAddress}:{}),addressLocality:place.addressLocality,addressCountry:place.addressCountry}})):{'@type':'Place',name:data.eventLocation?.name||'Venezia',address:{'@type':'PostalAddress',...(data.eventLocation?.streetAddress?{streetAddress:data.eventLocation.streetAddress}:{}),addressLocality:data.eventLocation?.addressLocality||'Venezia',addressCountry:data.eventLocation?.addressCountry||'IT'}},
