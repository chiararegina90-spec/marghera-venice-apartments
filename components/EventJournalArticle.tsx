...
const ui:Record<EventLang,{home:string;journal:string;tip:string;official:string;sourceNote:string;more:string;back:string;date:string;breadcrumb:string;faq:string}>={
  it:{home:'Home',journal:'Journal',tip:'Il consiglio di Marghera Venice Apartments',official:'Informazioni ufficiali',sourceNote:'Programma, orari, disponibilità e condizioni possono cambiare. Per le informazioni variabili controlla sempre la fonte ufficiale prima della visita.',more:'Potrebbe interessarti anche',back:'Torna al Journal',date:'Quando',breadcrumb:'Percorso di navigazione',faq:'Domande frequenti'},
  en:{home:'Home',journal:'Journal',tip:'Marghera Venice Apartments tip',official:'Official information',sourceNote:'Programmes, times, availability and conditions may change. Always check variable information with the official source before your visit.',more:'You may also like',back:'Back to Journal',date:'When',breadcrumb:'Breadcrumb',faq:'Frequently asked questions'},
  de:{home:'Startseite',journal:'Journal',tip:'Tipp von Marghera Venice Apartments',official:'Offizielle Informationen',sourceNote:'Programm, Uhrzeiten, Verfügbarkeit und Bedingungen können sich ändern. Variable Angaben bitte vor dem Besuch bei der offiziellen Quelle prüfen.',more:'Das könnte Sie auch interessieren',back:'Zurück zum Journal',date:'Termin',breadcrumb:'Brotkrümelnavigation',faq:'Häufige Fragen'},
  fr:{home:'Accueil',journal:'Journal',tip:'Le conseil de Marghera Venice Apartments',official:'Informations officielles',sourceNote:'Programme, horaires, disponibilités et conditions peuvent évoluer. Vérifiez toujours les informations variables auprès de la source officielle.',more:'À découvrir aussi',back:'Retour au Journal',date:'Quand',breadcrumb:'Fil d’Ariane',faq:'Questions fréquentes'},
  es:{home:'Inicio',journal:'Journal',tip:'Consejo de Marghera Venice Apartments',official:'Información oficial',sourceNote:'El programa, los horarios, la disponibilidad y las condiciones pueden cambiar. Comprueba siempre la información variable en la fuente oficial.',more:'También puede interesarte',back:'Volver al Journal',date:'Cuándo',breadcrumb:'Ruta de navegación',faq:'Preguntas frecuentes'},
  zh:{home:'首页',journal:'旅行日志',tip:'Marghera Venice Apartments 建议',official:'官方信息',sourceNote:'节目、时间、余位和条件可能变化。涉及实时信息时，请在出行前通过官方来源再次确认。',more:'你可能也感兴趣',back:'返回旅行日志',date:'日期',breadcrumb:'面包屑导航',faq:'常见问题'}
};
...
export default function EventJournalArticle({lang,data}:{lang:EventLang;data:EventArticle}){
  ...
  return <><Header lang={lang}/><main><article className="editorial-page">
    ...
    <section className="py-16 sm:py-20"><div className="mx-auto max-w-4xl px-5 lg:px-8"><div className="mb-10 flex flex-wrap items-center gap-3 border-y border-slate-200 py-4 text-sm font-bold text-navy"><span className="text-gold">{t.date}</span><span>{data.eventDate}</span><span className="text-slate-300">•</span><span>Venezia</span></div><p className="font-serif text-[clamp(1.65rem,3vw,2.25rem)] leading-[1.42] text-navy">{data.lead}</p><div className="mt-14 space-y-12">{data.sections.map(([title,text],i)=>{const links=sectionLinks.filter(link=>link.section===i);return <section key={title} className="editorial-section-row grid grid-cols-[34px_1fr] gap-3 sm:grid-cols-[42px_1fr] sm:gap-4"><div className="font-serif text-5xl text-gold">{String(i+1).padStart(2,'0')}</div><div><h2 className="font-serif text-[clamp(2rem,4vw,2.6rem)] leading-tight text-navy">{title}</h2><p className="mt-4 text-lg leading-8 text-slate-600">{text}</p>{links.map((link,j)=>{const external=/^https?:\/\//.test(link.href);return <p key={`${link.href}-${j}`} className="mt-4 text-lg leading-8 text-slate-600">{link.text}{external?<a href={link.href} target="_blank" rel="noopener noreferrer" className="font-bold text-navy underline decoration-gold decoration-2 underline-offset-4">{link.label}</a>:<Link href={localePath(link.href,lang)} className="font-bold text-navy underline decoration-gold decoration-2 underline-offset-4">{link.label}</Link>}{link.tail||''}</p>})}</div></section>})}</div>{data.internalLink&&<p className="mt-10 text-lg leading-8 text-slate-600">{data.internalLink.text}<Link href={data.internalLink.href} className="font-bold text-navy underline decoration-gold decoration-2 underline-offset-4">{data.internalLink.label}</Link>{data.internalLink.tail}</p>}</div></section>
    ...
    {faq.length>0&&<section className="py-14 sm:py-16"><div className="mx-auto max-w-4xl px-5 lg:px-8"><p className="text-xs font-black uppercase tracking-[.2em] text-gold">FAQ</p><h2 className="mt-3 font-serif text-4xl text-navy sm:text-5xl">{t.faq}</h2><div className="mt-7 space-y-3">{faq.map(item=><details key={item.question} className="group rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><summary className="cursor-pointer list-none pr-8 font-bold text-navy marker:content-none">{item.question}<span className="float-right text-xl text-gold group-open:rotate-45">+</span></summary><p className="mt-4 leading-7 text-slate-600">{item.answer}</p></details>)}</div></div></section>}
    <EditorialStayCta lang={lang} context="journal" copyOverride={data.stayCta}/>
