...
const ui:Record<Lang,{home:string;journal:string;more:string;sourceNote:string}>={
 de:{home:'Startseite',journal:'Journal',more:'Weitere Tipps für Ihren Aufenthalt',sourceNote:'Termine, Preise und Bedingungen können sich ändern. Variable Angaben bitte kurz vor dem Besuch bei der offiziellen Quelle prüfen.'},
 fr:{home:'Accueil',journal:'Journal',more:'D’autres conseils pour votre séjour',sourceNote:'Dates, tarifs et conditions peuvent évoluer. Vérifiez les informations variables auprès de la source officielle avant votre visite.'},
 es:{home:'Inicio',journal:'Journal',more:'Más consejos para tu estancia',sourceNote:'Las fechas, precios y condiciones pueden cambiar. Comprueba la información variable en la fuente oficial antes de la visita.'},
 zh:{home:'首页',journal:'旅行日志',more:'更多住宿期间的实用内容',sourceNote:'日期、票价和条件可能变化。涉及实时信息时，请在出行前通过官方来源再次确认。'}
};
...
export default function AuditedLocalizedJournalArticle({lang,data,tipLabel,officialLabel,backLabel}:{lang:Lang;data:JournalArticleData;tipLabel:string;officialLabel:string;backLabel:string}){
 ...
 return <><Header lang={lang}/><main><article className="editorial-page">{schemas.map((schema,i)=><script key={i} type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(schema)}}/>)}
  <EditorialHero image={data.image} imageAlt={data.imageAlt} crumbs={[{label:t.home,href:localePath('/',lang)},{label:t.journal,href:localePath('/journal',lang)},{label:data.title}]} eyebrow={<>{data.category}{data.eventDate&&<> • {data.eventDate}</>}</>} title={data.title} subtitle={data.description} strongShade={Boolean(data.posterHero)}/>
