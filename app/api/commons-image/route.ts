...
export async function GET(request:Request){
  ...
  const exact=/^File:/i.test(query);
  if(exact){
    api.searchParams.set('titles',query);
  }else{
  ...
  try{
    ...
    const candidates=pages.map((p:any)=>({title:String(p.title||''),info:p.imageinfo?.[0]})).filter((x:any)=>x.info&&['image/jpeg','image/png','image/webp'].includes(x.info.mime)&&x.info.width>=650);
    const ranked=exact?candidates:candidates.sort((a:any,b:any)=>{
