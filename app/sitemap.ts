...
const commonLogicalRoutes=['/','/case/rossi-apartment','/case/dimora-castelli','/come-raggiungere-venezia','/appartamenti-venezia-parcheggio-gratuito','/faq','/collaborazioni','/collaborazioni/row-venice','/collaborazioni/i-campi','/link-utili','/privacy-policy','/cookie-policy','/scopri-venezia','/scopri-il-veneto','/guide','/journal','/dove-mangiare','/servizi-in-zona'] as const;
...
export default function sitemap():MetadataRoute.Sitemap{
 ...
 const entries=logicalRoutes.flatMap(logical=>siteLangs.map(lang=>{const priority=logical==='/'?1:logical.startsWith('/case/')?0.9:logical==='/appartamenti-venezia-parcheggio-gratuito'?0.9:logical==='/come-raggiungere-venezia'?0.8:logical.startsWith('/journal/')?0.78:logical.startsWith('/guide/')?0.76:0.7;const frequency=logical==='/'||logical==='/journal'||logical.startsWith('/journal/')?'weekly' as const:'monthly' as const;return item(logical,lang,priority,frequency);}));
