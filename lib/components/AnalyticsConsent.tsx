...
function ensureMetaPixel(onReady?:()=>void){
  if(!window.fbq){
  ...
  if(window.__mvaMetaLoaded){
    onReady?.();
    return;
  }

  const existing=document.getElementById('mva-meta-pixel') as HTMLScriptElement|null;
  const ready=()=>{
    window.__mvaMetaLoaded=true;
    onReady?.();
  };
  if(existing){
...
export default function AnalyticsConsent({lang}:{lang:Lang}){
  ...
  useEffect(()=>{
    ...
    if(preferences.marketing){
      ...
      if(category) trackMeta('ViewContent',{content_name:document.title,content_category:category,content_ids:[pathname],content_type:'product'});
    }
  ...
  useEffect(()=>{
    ...
    const handler=(event:MouseEvent)=>{
      ...
      if(preferences.marketing) trackMeta('Contact',{content_name:method,content_category:'direct_contact'});
    };
