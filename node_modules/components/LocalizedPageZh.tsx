...
export default function LocalizedPageZh({
  title,
  subtitle,
        ...
        <section className="relative min-h-[68vh] overflow-hidden pt-20">
          ...
          <div className="relative mx-auto flex min-h-[calc(68vh-5rem)] max-w-7xl items-end px-5 pb-16 pt-20 lg:px-8">
            <div className="max-w-4xl text-white">
              <Link href="/zh" className="text-sm text-white/70">
                首页
              </Link>
              <h1 className="mt-5 break-words font-serif text-5xl leading-[1.05] sm:text-6xl md:text-8xl">
                {title}
              </h1>
                  ...
                  <div className="mt-6 flex flex-wrap gap-3">
                    {section.links.map(([label, href]) => (
                      <Link
                        key={href}
                        href={href}
                        className="max-w-full break-words rounded-full border border-navy px-5 py-3 font-bold text-navy"
                      >
                        {label}
                      </Link>
                    ))}
